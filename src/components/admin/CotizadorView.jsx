...
`;
  ...
  const handlePortToOnboarding = () => {
    ...
    onImportToOnboarding({
      ...
      requirements: `Setup: $${setupVal.toLocaleString()} | Mensualidad: $${mensualidadVal.toLocaleString()} | Comisión: ${comisionVal}%\n\nResumen: ${resumenPropuesta}`,
      branding: {
        primaryColor: '#4f46e5',
              ...
              <div key={idx} className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] p-5 rounded-2xl space-y-3">
                ...
                <div className="space-y-2 mt-2">
                  {paso.preguntas[0].opciones.map((opt, oIdx) => (
                    <label 
                      key={oIdx}
                      className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${scores[paso.preguntas[0].id] === opt.pts ? 'bg-indigo-950/20 border-indigo-500/40 text-indigo-300' : 'bg-[var(--color-bg)]/40 border border-[var(--color-border)] text-[var(--color-text-muted)] hover:bg-[var(--color-surface-2)]/55'}`}
                    >
                      <input 
                        type="radio"
                      ...
                      <div className="flex-1 text-xs">
                        <span className="font-semibold block text-[var(--color-text)]">{opt.label}</span>
                      </div>
                      <span className="text-[10px] font-black bg-[var(--color-surface-2)] px-2 py-0.5 rounded-lg text-[var(--color-text-muted)] shrink-0">
                        {opt.pts} pts
                      </span>
                    </label>
                  ))}
          ...
          <div className="w-1/2 overflow-y-auto p-6 space-y-6 flex flex-col justify-between bg-[var(--color-bg)]/10">
            <div className="space-y-6">
              ...
              <div className="bg-[var(--color-bg)]/50 border border-[var(--color-border)] p-6 rounded-2xl flex justify-between items-center">
                ...
                <div className="bg-indigo-950/20 border border-indigo-500/20 p-4 rounded-xl text-center min-w-[120px]">
                  <span className="text-[9px] uppercase font-bold text-indigo-400 tracking-wider">Setup Base Recomendado</span>
                  <div className="text-xs font-black text-[var(--color-text)] mt-2">
