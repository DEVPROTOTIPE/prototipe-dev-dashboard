...
export default function CreditosSaldosSandbox() {
  ...
  const handleMakePayment = () => {
    ...
    if (hasError) {
      showAlert({
        title: 'Error de Abono',
        message: 'El monto del abono supera la deuda actual.',
        variant: 'error'
      });
      return;
    }
  ...
  return (
    <div className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-2xl p-4 space-y-4 max-w-md mx-auto text-slate-200">
      ...
      <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1 scrollbar-thin">
        {clients.map(c => (
          <div key={c.id} className="bg-[var(--color-surface)]/40 p-2.5 rounded-xl border border-[var(--color-border)]/80 flex items-center justify-between text-xs">
      ...
      <div className="pt-2 border-t border-[var(--color-border)] space-y-2 text-xs">
        ...
        <div className="flex gap-2">
          ...
          <input
            ...
            className="w-24 bg-[var(--color-surface)] border border-[var(--color-border)] text-slate-200 rounded-xl px-2 py-1 focus:outline-none font-mono text-[11px]"
          />
          <button
