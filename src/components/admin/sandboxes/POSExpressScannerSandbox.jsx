...
export default function POSExpressScannerSandbox() {
  ...
  const processBarcode = (code) => {
    ...
    if (prod) {
      ...
      showAlert({
        ...
        message: `Código o SKU "${trimmed}" no encontrado.\nPrueba con los SKU sugeridos.`,
        variant: 'error'
      });
    }
    setBarcodeInput('');
  ...
  return (
    <div className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-2xl p-4 space-y-4 max-w-md mx-auto text-slate-200">
      <div>
      ...
      <div className="grid grid-cols-3 gap-2 bg-[var(--color-surface)]/40 p-2.5 rounded-xl border border-[var(--color-border)] text-[9px]">
        {products.map(p => (
          <button
            key={p.id}
            onClick={() => processBarcode(p.barcode)}
            className="p-1 bg-[var(--color-bg)] hover:bg-slate-800 border border-[var(--color-border)] rounded text-[8.5px] cursor-pointer text-left flex flex-col justify-between h-11"
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] text-xs rounded-xl pl-8 pr-3 py-1.5 text-slate-200 focus:outline-none focus:border-indigo-500 font-mono"
          />
        </div>
        <button
      ...
      <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1 scrollbar-thin">
        {cart.length === 0 ? (
          ...
          cart.map(item => (
            <div key={item.id} className="flex items-center justify-between bg-[var(--color-surface)]/30 p-2 rounded-xl border border-[var(--color-border)]/60 text-xs">
              <div>
          ...
          <button
            onClick={() => {
              ...
              showAlert({
                ...
                message: '¡Venta registrada y cobro procesado exitosamente en la caja diaria!',
                variant: 'success'
              });
              setCart([]);
              setLastScanned(null);
