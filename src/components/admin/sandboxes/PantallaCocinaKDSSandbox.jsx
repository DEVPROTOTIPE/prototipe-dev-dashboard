...
export default function PantallaCocinaKDSSandbox() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center bg-[var(--color-surface)]/40 border border-[var(--color-border)] p-3 rounded-2xl">
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {Object.entries(columns).map(([statusKey, col]) => (
          <div key={statusKey} className="bg-[var(--color-surface)]/20 border border-[var(--color-border)]/60 rounded-2xl p-3 flex flex-col min-h-[300px]">
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 scrollbar-thin max-h-[400px]">
              {col.list.length === 0 ? (
                ...
                col.list.map(order => (
                  <div key={order.id} className="bg-[var(--color-bg)]/80 border border-[var(--color-border)] rounded-xl p-3 space-y-2 shadow-md">
