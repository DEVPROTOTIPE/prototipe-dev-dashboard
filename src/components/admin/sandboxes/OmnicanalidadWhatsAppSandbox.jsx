...
export default function OmnicanalidadWhatsAppSandbox() {
  ...
  const handleTestRedirect = () => {
    ...
    showAlert({
      ...
      message: `Línea: ${chan.label} (${chan.phone})\nURL: ${link}`,
      variant: 'success'
    });
  };
      ...
      <div className="space-y-3">
              ...
              <button
                ...
                className={`p-2 border rounded-xl text-left transition-all cursor-pointer ${
                  selectedChannel === c.id
                    ...
                    : 'bg-[var(--color-surface)] border-[var(--color-border)] text-slate-400 hover:text-slate-300'
                }`}
              >
        ...
        <div className="space-y-1 text-xs">
          ...
          <textarea
            ...
            className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 h-20 resize-none font-sans"
          />
        </div>
