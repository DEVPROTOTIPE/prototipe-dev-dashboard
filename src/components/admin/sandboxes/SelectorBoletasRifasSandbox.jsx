...
function SandboxRaffleNumberSelector({
  ...
  const triggerLuckyDraw = (count = 1) => {
    ...
    if (available.length < count) {
      showAlert({
        title: 'Números Agotados',
        message: 'No hay suficientes números libres disponibles para realizar el sorteo automático.',
        variant: 'error'
      });
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
        <div className="flex-1 w-full">
          ...
          {!isAdmin && selectedNumbers.length > 0 && (
            <div className="mt-4 p-4 bg-[var(--color-bg)]/50 border border-white/5 rounded-2xl flex items-center justify-between animate-fade-in">
              <div>
              ...
              <div className="flex flex-col gap-2 min-w-[160px] self-center">
                <button 
                  onClick={() => showAlert({
                    ...
                    message: `Procediendo a comprar las boletas: ${selectedNumbers.sort().join(', ')}`,
                    variant: 'success'
                  })}
        ...
        {isAdmin && (
          <div className="w-full lg:w-72 bg-[var(--color-bg)]/40 border border-white/5 p-5 rounded-3xl min-h-[300px] flex flex-col justify-between self-stretch">
            {crmDetail ? (
                  ...
                  <button
                    onClick={async () => {
                      const confirmed = await showConfirm({
                        ...
                        message: `¿Estás seguro de que deseas liberar la boleta #${crmDetail.number}?`,
                        variant: 'warning',
                  ...
                  <button
                    onClick={() => {
                      showAlert({
                        ...
                        message: `Enviando comprobante de boleta #${crmDetail.number} al WhatsApp: ${crmDetail.phone}`,
                        variant: 'success'
                      });
                    }}
