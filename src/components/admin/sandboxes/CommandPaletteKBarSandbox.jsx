...
function SandboxCommandPaletteKBar({ isOpen, onClose, commands = [] }) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[var(--color-surface)]/95 border border-[var(--color-border)] rounded-3xl shadow-2xl flex flex-col h-[360px] mt-[10vh] overflow-hidden z-10 text-slate-100 text-left">
              ...
              return (
                <div key={cmd.id}>
                  ...
                  >
                    ...
                    {cmd.shortcut && (
                      <span className="font-mono text-[9px] text-slate-500 bg-[var(--color-bg)] px-2 py-0.5 rounded border border-[var(--color-border)]">{cmd.shortcut}</span>
                    )}
                  </div>
                </div>
              );
