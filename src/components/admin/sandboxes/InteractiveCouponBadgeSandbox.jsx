    ...
    <div className="w-full space-y-2 text-left">
      {status === 'success' && appliedCoupon ? (
        <div className="flex items-center justify-between p-3.5 bg-emerald-500/10 border border-emerald-500/25 rounded-2xl animate-fade-in-up text-slate-100">
          ...
          <button onClick={() => { setStatus('idle'); setCode(''); setAppliedCoupon(null); onRemove(); }} className="px-2 py-1 bg-slate-900 border border-slate-800 text-[10px] font-bold text-red-400 rounded-lg hover:bg-slate-800 transition-all cursor-pointer border-none">Eliminar</button>
        </div>
      ) : (
