...
export default function BriefingStudioView({ dbInstance, showToast, onImportToOnboarding }) {
  ...
  const loadSessions = async () => {
    ...
    try {
      const q = collection(dbInstance, 'briefings');
  ...
  const handlePortToOnboarding = () => {
    ...
    onImportToOnboarding({
      projectName: form.seccion1.nombre,
      requirements: analysisResult.resumenEjecutivo,
      branding: {
        primaryColor: form.seccion14.colorPrimario,
        secondaryColor: form.seccion14.colorSecundario,
        googleFont: form.seccion14.googleFont
      },
      flags: analysisResult.recommendedFlags
    });
  };
        ...
        <div className="flex items-center gap-3 text-xs min-w-[280px]">
          ...
          {selectedSessionId && (
            <button 
              ...
              className="p-1.5 bg-red-950/30 hover:bg-red-900/40 border border-red-900/30 text-red-400 rounded-xl cursor-pointer transition-all active:scale-95 shrink-0 focus:outline-none focus:ring-0 flex items-center justify-center"
          ...
          <button 
            ...
            className="px-2.5 py-1.5 bg-indigo-950/40 hover:bg-indigo-900/60 text-indigo-400 rounded-xl border border-indigo-500/30 cursor-pointer transition-all active:scale-95 shrink-0 focus:outline-none focus:ring-0 flex items-center gap-1.5 font-bold"
          ...
          <button 
            ...
            className="px-2.5 py-1.5 bg-red-950/30 hover:bg-red-900/50 text-red-400 rounded-xl border border-red-900/30 cursor-pointer transition-all active:scale-95 shrink-0 focus:outline-none focus:ring-0 flex items-center gap-1.5 font-bold"
          ...
          <button 
            ...
            className="px-2 py-1.5 bg-[var(--color-surface-2)] hover:bg-slate-800 rounded-xl border border-[var(--color-border)] text-[var(--color-text)] cursor-pointer transition-all active:scale-95 shrink-0 focus:outline-none focus:ring-0"
          >
            Actualizar
          </button>
        </div>
          ...
          <>
            ...
            <div className="w-80 border-r border-[var(--color-border)] bg-[var(--color-bg)]/20 overflow-y-auto p-4 flex flex-col gap-2">
              <div className="mb-4">
                ...
                <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden mt-1 border border-[var(--color-border)]">
                  <div 
                    className="bg-indigo-500 h-full transition-all duration-300" 
                    style={{ width: `${(currentSection / 20) * 100}%` }}
                  />
                </div>
                <span className="text-[10px] text-indigo-400 font-bold block mt-1">Sección {currentSection} de 20 ({Math.round((currentSection / 20) * 100)}%)</span>
              </div>
              ...
              {SECCIONES.map(sec => (
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl transition-all border flex items-start gap-2.5 ${currentSection === sec.id ? 'bg-indigo-950/30 border-indigo-500/40 text-indigo-300' : 'bg-[var(--color-bg)]/40 border-transparent hover:bg-[var(--color-surface-2)] hover:text-[var(--color-text)] text-[var(--color-text-muted)]'}`}
                >
                  <span className={`w-5 h-5 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0 ${currentSection === sec.id ? 'bg-indigo-650 text-white' : 'bg-slate-800 text-[var(--color-text-muted)]'}`}>
                    {sec.id}
                  </span>
                  <div>
                    <h4 className="text-xs font-bold tracking-wide">{sec.title}</h4>
                    <p className="text-[9px] text-slate-500 truncate mt-0.5">{sec.desc}</p>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="bg-[var(--color-surface-2)]/40 border border-[var(--color-border)] p-6 rounded-2xl space-y-4">
                ...
                <div className="space-y-2 text-xs max-h-40 overflow-y-auto">
                  {analysisResult.suggestedComponents.map((comp, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 bg-[var(--color-bg)]/60 border border-[var(--color-border)] rounded-xl">
                      <div>
                        <span className="font-bold text-[var(--color-text)] block">{comp.name}</span>
                        <span className="text-[9px] text-slate-500 block">{comp.technicalName}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4 space-y-3 bg-[var(--color-bg)]/40 p-4 rounded-2xl border border-[var(--color-border)] text-[11px] font-mono">
                  <div>
                    ...
                    <div className="flex items-center gap-2 bg-[var(--color-surface-2)] px-2.5 py-1.5 rounded-lg border border-[var(--color-border)]">
                      ...
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(exportedPaths.briefing);
                          showToast('Ruta copiada ✓', { type: 'success' });
                        }}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 bg-[var(--color-surface-2)] px-2.5 py-1.5 rounded-lg border border-[var(--color-border)]">
                      ...
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(exportedPaths.analisis);
                          showToast('Ruta copiada ✓', { type: 'success' });
                        }}
                        className="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 rounded text-[9px] text-[var(--color-text)] cursor-pointer"
                      >
