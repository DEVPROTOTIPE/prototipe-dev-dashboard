            ...
            {filteredClientes.length === 0 ? (
              ...
              filteredClientes.map(c => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border flex items-center gap-3.5 transition-all ${
                    selectedClientId === c.id
                      ? 'bg-indigo-950/20 border-indigo-500/40 text-indigo-300 shadow-md'
                      : 'bg-[var(--color-bg)]/40 border-transparent hover:bg-[var(--color-surface-2)] hover:text-[var(--color-text)] text-[var(--color-text-muted)]'
                  }`}
                >
                  <Users size={15} className={`shrink-0 ${selectedClientId === c.id ? 'text-indigo-400' : 'text-slate-500'}`} />
                  <div className="min-w-0">
                    <h4 className="text-xs font-bold tracking-wide truncate">{c.nombre}</h4>
                    <span className="text-[9px] text-slate-500 font-semibold block truncate">{c.id}</span>
                  </div>
                </button>
              ))
            )}
              ...
              <div className="flex justify-between items-center bg-[var(--color-surface-2)]/30 p-5 rounded-2xl border border-[var(--color-border)]">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="px-3 py-1.5 bg-indigo-950/40 hover:bg-indigo-900/40 border border-indigo-900/30 text-indigo-400 rounded-xl text-[10px] font-bold flex items-center gap-2 cursor-pointer transition-all active:scale-95"
                  >
                    <CheckSquare size={12} /> Habilitar Todas
                  </button>
                  <button 
                    onClick={() => handleBulkAction('disable')}
                    className="px-3 py-1.5 bg-red-950/20 hover:bg-red-900/30 border border-red-900/20 text-red-400 rounded-xl text-[10px] font-bold flex items-center gap-2 cursor-pointer transition-all active:scale-95"
                  >
                    <XSquare size={12} /> Desactivar Todas
                  </button>
                </div>
              </div>
            )}
