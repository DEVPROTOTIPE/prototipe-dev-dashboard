...
const allowIntegritySync = process.env.PROTOTIPE_ALLOW_INTEGRITY_SYNC === '1';
...
function writeJsonAtomic(filePath, data) {
  if (!allowIntegritySync) {
    throw new Error('La verificación es de solo lectura. Define PROTOTIPE_ALLOW_INTEGRITY_SYNC=1 para autorizar la sincronización.');
  }
...
const rootCandidates = [
  process.env.PROTOTIPE_ROOT,
  path.resolve(devDashboardDir, '..', '..'),
  path.resolve(devDashboardDir, '..', 'PROTOTIPE'),
].filter(Boolean);
const rootDir = rootCandidates.find((candidate) =>
  fs.existsSync(path.join(candidate, 'Documentacion PROTOTIPE', '06_Biblioteca_Componentes', 'README.md'))
) || rootCandidates[0];
...
const hasDocs = fs.existsSync(docsRoot) && fs.existsSync(libraryReadmePath);
const isStandalone = process.env.DASHBOARD_STANDALONE_BUILD === '1' || !hasDocs;
...
if (isStandalone) {
  console.log('⚠️  Documentacion PROTOTIPE no encontrada — saltando validaciones documentales, build autónomo.');
}
...
try {
  ...
  if (!isStandalone) {
    ...
    allPhysicalFiles = [...compFiles, ...modFiles]
    ...
    console.log('\n[Info] Ejecutando linter estético y de robustez sobre fichas técnicas...');
  ...
  if (!isStandalone) {
    ...
    try {
      ...
      if (fs.existsSync(roadmapPath)) {
        ...
        let preservesPreexistingChanges = false;
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (match) {
            ...
            if (!isCompleted) {
              const idRegex = /(?:(CORE|CLI|CLAUDE|BUG|DASH|TPL|PLT|INST|DOC|LND|BIZ|HOTFIX|CLIENTE|E2E)-([A-Z0-9_-]+))/i;
              ...
              if (idMatch) {
                ...
                while (j < lines.length) {
                  ...
                  if (trimmed.match(/^-\s*Cambios preexistentes preservados:\s*s[ií]/i)) {
                    preservesPreexistingChanges = true;
                  }
        ...
        if (activeTaskId) {
          ...
          if (missingRegistries.length > 0 && preservesPreexistingChanges) {
            console.warn(`\n⚠️ [Alcance Multi-IA] ${missingRegistries.length} cambios locales no pertenecen al alcance declarado de ${activeTaskId} y permanecen visibles como preexistentes.`);
            console.warn('La tarea activa no los reclama, no los descarta y debe volver a comprobar el working tree antes de cada edición.');
            console.log(`[Éxito] El alcance propio de la tarea activa (${activeTaskId}) está declarado sin atribuir cambios preexistentes.`);
          } else if (missingRegistries.length > 0) {
  ...
  if (hasErrors) {
    ...
    console.log('\n==================================================');
    if (isStandalone) {
      console.log('  ✅ VERIFICACIÓN DE SEGURIDAD Y SANDBOXES COMPLETADA.');
    } else {
